...
const user = await db.select().from(users).where(eq(users.email, 'student@saveetha.in')).limit(1);
...
- **Ephemeral Data:** Bus locations are temporary — we only care about the _current_ position. If the server restarts, buses send fresh updates within seconds. No need to persist.
...
### 7. Frontend: React 19 + Vite + TailwindCSS 4 + Shadcn UI
...
- **Shadcn UI & Radix UI primitives:** Accessible, unstyled UI components tailored with Tailwind.
...
- **Shadcn UI:** Gives us beautiful, accessible components without the bloat of traditional component libraries. We own the component code completely.
...
- [Docker](https://www.docker.com/) (optional, for running out of the box)

### Option 1: Using Docker Compose (Recommended)

1. Clone the repository.
2. Ensure you have created the `.env` files in `backend/`, `frontend/`, and `batcher/` as described in the **Environment Variables** section below.
3. Run the following command from the root directory:
    ```bash
    docker-compose up --build
    ```
4. The frontend will be available at `http://localhost` and the backend at `http://localhost:3000`.

### Option 2: Manual Setup with Bun
...
1. **Backend:**
    ...
    bun run dev
    ```
2. **Batcher (Optional, for handling high traffic):**
    ```bash
    cd batcher
    bun install
    bun run dev
    ```
3. **Simulation (Optional, for testing with fake buses):**
    ```bash
    cd backend
    bun run sim
    ```
